...
def standings(date: date, league: str = 'AL') -> Standings:
    ...
    def pct(team):
        return wins.get(team, 0)/(wins.get(team, 0)+losses.get(team, 0))
    initialize()
    wins = {}
    losses = {}
    for g in gamelog:
    ...
    for k in losses.keys():
        codes.add(k)
    leader = max(wins, key=lambda k:pct(k))
    unsorted_standings = [Record(team, wins.get(team, 0), losses.get(team, 0), gb(team,leader)) for team in codes]
    return sorted(unsorted_standings, key=lambda s: (pct(s.code), -s.gb), reverse=True)
