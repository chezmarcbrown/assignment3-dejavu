...
Record = namedtuple("Team", "name code logo wins losses gb")
...
def standings(date: date, league: str = 'AL') -> Standings:
    ...
    unsorted_standings = [Record(team_name(team),team, team_logo(team), wins.get(team, 0), losses.get(team, 0), gb(team, leader)) for team in codes]
...
def extend_record(r: Record) -> ExtendedRecord:
    ...
    """
    return ExtendedRecord(team_names(r.code),r.code ,team_logos(r.code), team_name(r.code), team_logo(r.code), r.wins, r.losses, format_pct(r.wins, r.losses), r.gb)
...
def valid_date(current):
    start = start_of_season()
    end = end_of_season()
    res = False
    if((current.month >= start.month and current.month < end.month and current.year == start.year and current.year == end.year)):
        if((current.month == start.month and current.day >= start.day)):    # or (current.month == end.month and current.day <= end.day)):
            res = True
        elif(current.month != start.month):
            res = True
        else:
            res = False
    return res
def after_season(current):
    end = end_of_season()
    if(current.month >= end.month and current.year == end.year and current.day >= end.day):
        return True
    elif(current.year > end.year):
        return True
    return False
